...
2. Download the zip with the latest version of ReCap from the Releases tab here in Github ([Windows](https://github.com/vitor251093/resurrection-capsule/releases/latest/download/resurrection-capsule-windows-x64.zip)).

3. After that, you will need to find the DarksporeBin folder. Depending of your Darkspore version, its folder will be different:
...
4. Extract the contents of the zip of latest version of ReCap on the DarksporeBin folder.

5. Run the `RecapLauncher.exe` file, which is now inside the DarksporeBin folder. 

6. The launcher will launch the server and the game at the same time. Keep both running while playing.
